...
```
여기선 임의의 인덱스에 구조체 멤버에 넣을 값을 적는다.
구조체 크기는 `0x60`,  
...
근데 ~~tq~~ `i`에 대한 제한이 대놓고 없음ㅋㅋㅋㅋㅋㅋㅋ
**OOB** 확인~
...
`edit`또 딱히 `create`와 다른 점은 없다만, 그래서 취약점도 똑같이 존재함 ㅋㅋㅋㅋㅋㅋㅋㅋㅋ
...
각 구조체를 돌면서 `firstName`이 비어있지 않ㅇ느 경우에 해당 구조체의 값을 전부 출력한다.
